...
extern "C" JNIEXPORT void JNICALL Java_Jtest_showhomo (JNIEnv * env, jobject jo, jstring ff, jstring sf)
{
    ...
    const char* secondF = env->GetStringUTFChars(sf, NULL);
  Mat img_object = imread( firstF, CV_LOAD_IMAGE_GRAYSCALE );
  Mat img_scene = imread( secondF, CV_LOAD_IMAGE_GRAYSCALE );
  env->ReleaseStringUTFChars(ff, firstF);
  env->ReleaseStringUTFChars(sf, secondF);
